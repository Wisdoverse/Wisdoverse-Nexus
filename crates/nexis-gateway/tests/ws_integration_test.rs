//! WebSocket Integration Tests for Wisdoverse Nexus Gateway
//!
...
async fn ws_send_to_specific_user() {
    ...
    let sent = ws_state
        .send_to_user(
            "alice",
            axum::extract::ws::Message::Text("hello".to_string().into()),
        )
        .await;
    assert!(sent);
...
async fn ws_broadcast_to_all_connections() {
    ...
    ws_state
        .broadcast(axum::extract::ws::Message::Text(
            "broadcast".to_string().into(),
        ))
        .await;
...
async fn router_send_to_member() {
    ...
    let sent = router
        .send_to_member(&"alice".to_string(), "direct message")
        .await;
    assert!(sent);
    ...
    let sent = router
        .send_to_member(&"unknown".to_string(), "message")
        .await;
    assert!(!sent);
}
...
fn ai_extract_prompt_preserves_case() {
    ...
    assert!(AiHandler::extract_prompt("Hello World @ai How Are You?").contains("Hello World"));
...
async fn ws_user_in_multiple_rooms() {
    ...
    router
        ...
        .await;
    router
        .join_room(&"alice".to_string(), &"room_3".to_string())
        .await;
    ...
    // Leave one room
    router
        .leave_room(&"alice".to_string(), &"room_2".to_string())
        .await;
    let rooms: Vec<String> = router.member_rooms(&"alice".to_string()).await;
...
async fn ws_empty_room_is_removed() {
    ...
    router
        ...
        .await;
    router
        .leave_room(&"bob".to_string(), &"room_1".to_string())
        .await;
